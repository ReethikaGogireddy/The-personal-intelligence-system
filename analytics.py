...
# API 11 — Productivity prediction
@analytics_bp.route('/api/predict/productivity', methods=['GET', 'POST'])
def predict_productivity():
    if not prod_model:
        return jsonify({"error": "Run model.py first to train models."}), 500
    X, err = get_features()
    if err:
    ...
    return jsonify({
        ...
        "confidence": 0.82,
        "explanation": f"Your predicted productivity is {score}/10."
    })

# API 12 — Burnout risk prediction
...
def predict_burnout():
    ...
    return jsonify({
        "burnout_risk": label,
        "probability": prob,
        "explanation": f"Your burnout risk is {label} with {int(prob*100)}% confidence."
    })